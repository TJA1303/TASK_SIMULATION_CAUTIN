...
// Definición de la estructura que contendrá los datos
typedef struct {
    int identifier;
    float data;
    //float prom_temp;
...
//************************TASK DEL ADC****************/
void Get_prom_temp(void *pvParameter) //Tarea B
{
    while(true){
...
void task_uart (void *pvParameter)
{
    ...
    while(1){
        ...
        xQueueSend( queue, &temperas, pdMS_TO_TICKS(0) );
        //printf("SE ENVIO LA COLA \n");
        vTaskDelay(5000 / portTICK_RATE_MS); // Esperar un segundo para volver a medir la temperatura
    }
}

void task_increase_control (void *pvParameters) //Tarea A
{
    Temperatures temperas;
    //int cont = 0;
...
void task_control_threshold_button(void *pvParameters)
{
    ...
    while(1)
    {
        /*
        //Esperamos a recibir los datos de la cola
        if ( xQueueReceive(queue, &temperas, pdMS_TO_TICKS(1000)) == true )
        {
           //printf("COLA RECIBIDA Y EL THRESHOLD ES: %.2f \n",temperas.threshold_temp);
           //printf("COLA RECIBIDA Y LA HISTERESIS ES: %.2f \n",temperas.hist_temp);
           threshold_button = temperas.threshold_temp;
           //printf("COLA RECIBIDA Y EL THRESHOLD BUTTON ES: %.2f \n",temperas.threshold_temp);
           hist_button = temperas.hist_temp;
        }
        ...
        temperas.threshold_temp = threshold_button;
        temperas.hist_temp = hist_button;
        xQueueSend( queue, &temperas, pdMS_TO_TICKS(0) );
...
void app_main(void)
{
    ...
    queue = xQueueCreate(10, sizeof(Temperatures));
    //****************TASK**************************/
...
//Interrupcion para incrementar el threshold
void isr_handlerSUM(void *args)
{
    banderaSUM = 1;
}
...
//Interrupcion para decrementar el threshold
void isr_handlerRES(void *args)
{
    banderaRES = 1;
}